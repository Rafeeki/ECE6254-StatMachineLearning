...
k = [5, 25, 50, 250] # number of neighbors to consider 

for i in range(0,len(n)):

    ## Generate a simple 2D dataset
    X, y = datasets.make_moons(n[i],'True',0.3)

    ## Create instance of KNN classifier
    classifier = neighbors.KNeighborsClassifier(k[i],'uniform')
    classifier.fit(X, y)
    ...
    plt.title("%i-NN classifier trained on %i data points" % (k[i],n[i]))
